...
現在我們就來想想，平面上給定座標範圍最接近的兩個夾角可以多小？這個問題[在 Codefoces 的 Blog 上有某人給了解答](http://codeforces.com/blog/entry/17602)。但我用我自己的方法來解釋，因為兩向量夾角 $$\theta$$ 很小時，擁有公式 $$\theta \approx \sin{\theta} = \frac{\vec{v1} \times \vec{v2}}{|\vec{v1}|*|\vec{v2}|}$$，其中分子 $$\vec{v1} \times \vec{v2}$$由於座標都是整數又非零，所以最小的可能值為 $$1$$，而分母最大的可能情形為在限制範圍內最遠的兩個點座標距離平方，也就是 $$8*10^{12}$$，於是我們就能估出任兩個不同方向的最小夾角至少為 $$\frac{1}{8*10^{12}} \approx 1.25*10^{-13}$$